...
#define ALI_PRODUCTKEY "h23nYvb7IsK"//use ALi productkey
#define ALI_DEVICENEME "864284040282522"//can use IMEI to define different devices
#define ALI_PRODUCTSECRET "biGsJDKCl4yG7B8a"//use ALi productsecret

//!!!instanceId please refer to: https://help.aliyun.com/zh/iot/user-guide/mqtt-based-dynamic-registration?spm=a2c4g.11186623.0.0.45db811cRiRBo2
#define ALI_INSTANCEID "iot-06z00dlsf0ujzaq"
#define ALI_URL "iot-as-mqtt.cn-shanghai.aliyuncs.com"
#define ALI_PORT_REGISTER (443)//register connection for getting parameter of connection
#define ALI_PORT_CONNECT (1883)//final port of connection
...
extern void core_hmac_sha256(const uint8_t *msg, uint32_t msg_len, const uint8_t *key, uint32_t key_len, uint8_t output[32]);
...
BOOL mqtt_connect_server_get_connect_param(char *productkey, char *devicename, char *productsecret)
{
    ...
    
    {
        ...
        srand((unsigned)sAPI_Time(NULL) *10);
        random_num = rand();

        core_uint2str(random_num, random, NULL);
        sAPI_Debug("random = %s", random);
        
        char *client_src[] = {devicename, productkey, random, auth_type, ALI_INSTANCEID};
        char *username_fmt = "%s&%s";
        char *username_src[] = {devicename, productkey };
        char *sign_input_fmt = "deviceName%sproductKey%srandom%s";
        char *client_fmt = "%s.%s|random=%s,authType=%s,securemode=2,signmethod=hmacsha256,instanceId=%s|";
        char *client_id = NULL;
        char *usrName = NULL;
        ...
        //clientId
        core_sprintf(&client_id, client_fmt, client_src, 5);
        ...
        //host
        snprintf(host, sizeof(host)-1, "tcp://%s:%d", ALI_URL, ALI_PORT_REGISTER);
...
BOOL mqtt_connect_server_final(mqtt_conn_msgq_params *param)
{
    ...
    snprintf(host, sizeof(host)-1, "tcp://%s:%d", ALI_URL, ALI_PORT_CONNECT);
...
void mqtt_aliauth_dynreg(void *arg)
{
    ...
    while(1)
    {
        ret_Cgreg = sAPI_NetworkGetCgreg(&pGreg);
        if(1 != pGreg)
        {
            sAPI_Debug("MQTT NETWORK STATUS IS [%d] ret_Cgreg[%d]",pGreg, ret_Cgreg);
    ...
    {
        char *productkey = ALI_PRODUCTKEY;//use ALi productkey
        char *devicename = ALI_DEVICENEME;//can use IMEI to define different devices
        char *productsecret = ALI_PRODUCTSECRET;//use ALi productsecret
...
void sTask_MqttRecv(void *arg)                    //thread of sub topic data process
{
    while(1)
    {
        ...
        **=======================================================================================================================*/
		...
		PrintfRespData((char*)sub_data->payload_P,sub_data->payload_len);                                                                              //                     //    
        PrintfRespData((char*)"\r\n",2);                                                                                                               //                     //
        /*************************************************************************************/                                                         //                     //
