...
void TcpipDemo(void)
{
    ...
    while (1)
    {
        ...
        switch (opt)
        {
            ...
            {
                char *note = "\r\ninput ip and port,for example: 123.45.67.89:8888 or www.example.com:8888\r\n";
                char buf[100] = {0}, *serverIP = NULL;
                unsigned short port;
            ...
            case SC_CLIENT_CLOSE:
            {
                ...
                else
                {
                    ...
                    {
                        socket_errno = lwip_getsockerrno(sockfd);
                        ui_print("close client fail errno[%d]", socket_errno);
            ...
            case SC_TCPIP_DEMO_TCP_SERVER_OPEN:
            {
                ...
                if (fd > 0)
                {
                    ...
                    sraddr.sin_port = htons(6666);
                    bind(fd, (struct sockaddr *)&sraddr, sizeof(sraddr));
                    listen(fd, 5);
                    ui_print("accept....\r\n");
                    fdc = accept(fd, (struct sockaddr *)&claddr, &len);
                    if (fdc > 0)
                    {
                        ui_print("clinet family=[%d] addr=[%d]\r\n", claddr.sin_family, claddr.sin_addr.s_addr);
                    }
                    else
                    {
                        ui_print("accept err fdc=[%d]\r\n", fdc);
                    }
                }
                break;
            }
