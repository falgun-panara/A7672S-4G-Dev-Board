...
sMsgQRef uart_mqtt_msgq_1 = NULL;
...
void sTask_MqttRecvProcesser(void *arg)                    //thread of sub topic data process
{
    while (1)
    {
        ...
        **=======================================================================================================================*/
        ...
        sAPI_MsgQRecv(urc_mqtt_msgq_1, &msgQ_data_recv,
                      SC_SUSPEND);                                                                                    //                     //
        if ((SC_SRV_MQTT != msgQ_data_recv.msg_id) || (0 != msgQ_data_recv.arg1)
            || (NULL == msgQ_data_recv.arg3))  //wrong msg received                //                     //
            continue;                                                                                                                                   //                     //
        sub_data = (SCmqttData *)(
                       msgQ_data_recv.arg3);                                                                                                  //                     //
        /*this part just for test, the payload msg just ascii string*/                                                                                  //                     //
        sAPI_Debug("MQTT TEST----------index: [%d]; tpoic_len: [%d]; tpoic: [%s]", sub_data->client_index, sub_data->topic_len,
                   sub_data->topic_P);     //                     //
        //                     //
        sAPI_Debug("MQTT TEST----------payload_len: [%d]",
                   sub_data->payload_len);                                                                      //   rception cycle    //
        PrintfRespData((char *)"recieve topic: ", strlen("recieve topic: "));
        PrintfRespData((char *)sub_data->topic_P, sub_data->topic_len);
        PrintfRespData((char *)"\r\n", 2);
        PrintfRespData((char *)"recieve payload: ", strlen("recieve payload: "));
        PrintfRespData((char *)sub_data->payload_P,
...
void sAPP_MqttTaskDemo(void)
{
    ...
    **==================================================================================================================*/
    status = sAPI_MsgQCreate(&urc_mqtt_msgq_1, "urc_mqtt_msgq_1", (sizeof(SIM_MSG_T)), 4,
                             SC_FIFO);        //msgQ for subscribed data transfer
    if (status != SC_SUCCESS)
        sAPI_Debug("message queue creat err!\n");

    status = sAPI_MsgQCreate(&uart_mqtt_msgq_1, "uart_mqtt_msgq_1", (sizeof(SIM_MSG_T)), 4,
                             SC_FIFO);      //msgQ for subscribed data transfer
    if (status != SC_SUCCESS)
        sAPI_Debug("message queue creat err!\n");

    /*creat a thread*/
    if (sAPI_TaskCreate(&gMqttProcessTask, gMqttProcessTaskStack, SC_DEFAULT_THREAD_STACKSIZE * 2, SC_DEFAULT_TASK_PRIORITY,
                        (char *)"mqttProcesser", sTask_MqttProcesser, (void *)0)) //init the demo task
    {
        gMqttProcessTask = NULL;
        sAPI_Debug("Task Create error!\n");
    }

    if (sAPI_TaskCreate(&gMqttRecvTask, gMqttRecvTaskStack, SC_DEFAULT_THREAD_STACKSIZE * 2, SC_DEFAULT_TASK_PRIORITY,
                        (char *)"mqttProcesser", sTask_MqttRecvProcesser, (void *)0)) //init the demo task
    {
        gMqttRecvTask = NULL;
        sAPI_Debug("Task Create error!\n");
    }
...
extern void Get_Cfg_Parameters(char *product_id, char *device_name, char *device_secret, char *region,
                               char *cfg_client_id, char *cfg_host_addr, char *cfg_username, char *cfg_password);
/**
    ...
    while (1)
    {
        ...
        switch (opt)
        {
            ...
            case SC_MQTTS_DEMO_CONNECT:
                ...
                Get_Cfg_Parameters(productkey, devicename, devicesecret, region, cfg_client_id, cfg_host_addr, cfg_username,
                                   cfg_password);
            ...
            case SC_MQTTS_DEMO_SUBSCRIBE:
                ...
                topic_len = UartReadLine(topic, sizeof(topic));
            ...
            case SC_MQTTS_DEMO_PUBLISH:
                ...
                payload_len = UartReadLine(payload, sizeof(payload));
...
extern void Cfg_mqtt_service(char *product_ID, char *device_name, char *device_secret,  char *cfg_client_id,
                             char *cfg_host_addr, char *cfg_username, char *cfg_password);
/**
    ...
    while (1)
    {
        ...
        switch (opt)
        {
            ...
            case SC_MQTTS_DEMO_CONNECT:
                ...
                topic_len = UartReadLine(productkey, sizeof(productkey));

                PrintfResp("\r\nPlease input devicename\r\n");
                topic_len = UartReadLine(devicename, sizeof(devicename));

                PrintfResp("\r\nPlease input devicesecret\r\n");
                topic_len = UartReadLine(devicesecret, sizeof(devicesecret));
...
void MqttDemo(void)
{
    ...
    {
        if (sAPI_TaskCreate(&gMqttRecvTask, gMqttRecvTaskStack, SC_DEFAULT_THREAD_STACKSIZE * 2, SC_DEFAULT_TASK_PRIORITY,
                            (char *)"mqttProcesser", sTask_MqttRecvProcesser, (void *)0) != SC_SUCCESS) //init the demo task
        {
            sAPI_Debug("Task Create error!");
        }
    }
