...
typedef enum
{
    ...
    SC_UART_DEMO_SET_UART_RX_SLEEP  = 15,
    #endif
...
#ifdef UART_RX_WAKE_DEMO
            case SC_UART_DEMO_SET_UART_RX_SLEEP:
            {
                PrintfResp("\r\nPlease pull down the DTR pin to wake up the module.\r\n");
                PrintfResp("\r\nPlease unplug the USB to put the module into sleep mode.\r\n");
                if (sAPI_SystemSleepRxWakeSet(SC_SYSTEM_SLEEP_ENABLE) != 0)
                {
                    PrintfResp("\r\nSet Uart Rx sleep mode error.\r\n");
                }
                PrintfOptionMenu(options_list, sizeof(options_list) / sizeof(options_list[0]));
                goto reSelect;
                break;
