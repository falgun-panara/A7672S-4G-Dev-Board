...
#include "string.h"
#include "stdio.h"
#if 0 /**Echo suppression debugging*/
...
void AudioDemo(void)
{
    ...
    char *optionsList[] =
    {
        ...
        "18, stop amr buffer",
        "19, Play amr cont",
        "20, set amr encoder rate",
        "21. get amr record frame",
        "22. stop amr frame",
#endif
        "23. Play WAV with high sampling rate",
        "24. Play Wav cont",
    ...
    while (1)
    {
        ...
        switch (opt)
        {
            ...
            case SC_AUDIO_DEMO_PLAY_FILE:
            {
                ...
                UartReadLine(audioFile, sizeof(audioFile));

                PrintfResp("\r\nPlease input direct or not, 1 or 0\r\n");
                directPlay = UartReadValue();

                PrintfResp("\r\nPlease input is single file palyback, 1 or 0\r\n");
                isSingle = UartReadValue();
            ...
            case SC_AUDIO_DEMO_RECORD:
            {
                ...
                UartReadLine(recording_file, sizeof(recording_file));
            ...
            case SC_AUDIO_DEMO_SET_VOLUME:
            {
                ...
                volume = (AUD_Volume)UartReadValue();
            ...
            case SC_AUDIO_DEMO_SET_MICGAIN:
            {
                ...
                micgain = UartReadValue();
#ifdef FEATURE_SIMCOM_POC
                if (micgain < 0 || micgain > 23)
#else
...
#endif
            case SC_AUDIO_DEMO_PCM_RECORD:
            {
                fp = sAPI_fopen("c:/12.wav", "ab+");
                BOOL ret = sAPI_PCMStreamRec((void *)GetPCMRec);
                if (0 == ret)
                {
                    printf("start pcm record failed!");
                    break;
                }
                printf("start pcm record successfully!");
                break;
            }
            case SC_AUDIO_DEMO_STOP_PCM_RECORD:
            {
                BOOL ret = sAPI_PCMStopStreamRec();
                int rf = sAPI_fclose(fp);
                if (rf != 0)
                {
                    printf("close file failed! %d", rf);
                }
                if (0 == ret)
                {
                    printf("stop pcm record failed!");
                    break;
                }
                printf("stop pcm record successfully!");
                break;
            }
#if 0 /**Echo suppression debugging*/
            case 31:
